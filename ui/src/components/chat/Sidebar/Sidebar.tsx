...
const SidebarHeader = () => (
  <div className="flex items-center gap-2">
    ...
    <span className="text-xs font-medium uppercase text-white">MOPS Knowledge Lab</span>
  </div>
)
...
const Sidebar = ({ isMobileOpen, onMobileClose }: {
  ...
  return (
    <motion.aside
      className={`sidebar relative flex h-screen shrink-0 grow-0 flex-col overflow-hidden px-2 py-3 font-dmmono${isMobileOpen ? " mobile-open" : ""}`}
      initial={{ width: '16rem' }}
          ...
          <>
            {isEndpointLoading ? (
              ...
              <div className="text-xs text-muted">
                Backend not available. Check that the server is running.
              </div>
            )}
          </>
        )}
